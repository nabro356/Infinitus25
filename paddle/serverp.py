...
from langdetect import detect  # Auto-detect text language
...
# Initialize PaddleOCR (for Mode-I OCR)
...
def text_to_speech(text, filename, lang="en"):
    """Convert text to speech and save as an .mp3 file in the detected language"""
    tts = gTTS(text=text, lang=lang)
    tts.save(filename)

def apply_paddleocr(image_path):
    """Perform OCR using PaddleOCR and return extracted text"""
    results = ocr.ocr(image_path, cls=True)
    extracted_text = " ".join([res[1][0] for line in results for res in line])
...
def describe_scene_with_gemini(labels, image_path, person_name):
    ...
    if "Paper" in labels_str or "Document" in labels_str:
        extracted_text = apply_paddleocr(image_path)
    ...
    try:
        while True:
            ...
            if mode_data == "CAPTURE":
                ...
                text_to_speech(text, mp3_file_path, lang=detected_lang)
